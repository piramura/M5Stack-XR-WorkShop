...
M5Stackのタッチ入力に合わせてSK6812 LEDテープを点灯し、タッチイベントをUnityへOSC送信するワークショップ用プロジェクトです。
...
当日の進行は[当日スライド](SLIDE_URL)を正とします。以下は、作業中に詰まった場合の確認用です。

> `SLIDE_URL`は当日スライドの公開URLへ差し替えてください。
...
4. `src/main.cpp`を開き、次の行のコメントを外してOSCを有効にします。
